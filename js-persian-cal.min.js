...
AMIB.persianCalendar.prototype.createDateTable = function (b, a, c) {
    ...
    //by labbaf
    var PreYearLeap = a[0] - 1;
    var isLeap = leap_persian(PreYearLeap);
    var startDay = 3;
    if (isLeap)
        startDay = 4;

    var l = Math.floor(persian_to_jd(a[0], a[1], 1) + startDay) % 7;
    e = d.insertRow(-1);
    g = e.insertCell(-1);
...
AMIB.persianCalendar.prototype.SetHoliday = function () {
    ...
    if (typeof(DateHoliday) !== 'undefined')
...
function leap_persian(jy) {
    ...
    let nYears = jy - jp;
    leapJ = leapJ + Math.floor(nYears / 33) * 8 + Math.floor(((nYears % 33) + 3) / 4);

    if ((jump % 33) === 4 && (jump - nYears) === 4) leapJ++;

    let leapG = Math.floor(gy / 4) - Math.floor(((Math.floor((gy / 100) + 1)) * 3) / 4) - 150;
    let march = 20 + leapJ - leapG;

    let isLeap = (((nYears + 1) % 33) - 1) % 4 === 0;
...
function jd_to_persian(b) {
    ...
    //b = b - persian_to_jd(a, c, 1)+1; //by labbaf
    b = b - persian_to_jd(a, c, 1);
...
function persian_to_jd(b, a, c) {
    ...
    b -= 0 <= b ? 474 : 473;
    d = 474 + mod(b, 2820);
    var res = c + (7 >= a ? 31 * (a - 1) : 30 * (a - 1) + 6) + Math.floor((682 * d - 110) / 2816) + 365 * (d - 1) + 1029983 * Math.floor(b / 2820) + (PERSIAN_EPOCH - 1)
    return res;
}
...
function persianMonthDays(b, a) {
    var res = 6 >= a ? 31 : 11 >= a ? 30 : 12 == a ? leap_persian(b) ? 30 : 29 : 0;
    return res 
}
